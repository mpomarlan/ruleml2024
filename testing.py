...
def updateTheory(oldRule, theory, newRule):
    if oldRule not in theory:
        return None
    moreSpecific = (0 != len(newRule[0].difference(oldRule[0])))
    k = theory.index(oldRule)
    kA = k + 1
    if newRule in theory:
        kN = theory.index(newRule)
        if k > kN:
            if moreSpecific:
                theoryR = theory[:kN] + theory[kN+1:k+1] + [newRule] + theory[k+1:]
                kA = k + 1
            else:
                theoryR = theory[:kN] + theory[kN+1:k] + [newRule] + theory[k+1:]
                kA = k
        else:
            #print("kN already above k", oldRule, newRule)
            theoryR = theory
    else:
        if moreSpecific:
            theoryR = theory[:k+1] + [newRule] + theory[k+1:]
            kA = k + 1
        else:
            theoryR = theory[:k] + [newRule] + theory[k+1:]
            kA = k
    for k, e in enumerate(theoryR):
        if k < len(theory):
            theory[k] = e
        else:
            theory.append(e)
